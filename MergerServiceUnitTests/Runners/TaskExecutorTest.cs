...
namespace MergerLogicUnitTests.Utils
{
  ...
  public class TaskExecutorTest
  {
    ...
    public void WhenConfiguringBatchLimits_ShouldWriteTilesEachTimeAfterReachingBatchLimit(bool limitBatchSize, int batchMaxSize, long batchMaxBytes, int amountOfFlushes, int totalAmountOfTiles)
    {
      ...
      {
        sourceDataMock.Setup(sourceData => sourceData.GetCorrespondingTile(testSourceCoords[testSourceCoordIdx], It.IsAny<TileFormat>(), It.IsAny<bool>())).Returns(testSourceTiles[testSourceCoordIdx]);
      }
    ...
    private Tuple<MergeTask, Mock<IData>, Tile[]> SetupTestTask(int amountOfSources, bool isTargetNew)
    {
      ...
      Mock<IData>[] sourcesDataMocks = testSourcesTiles.Select((tile, index) =>
      {
        ...
        sourceDataMock.Setup(sourceData => sourceData.GetCorrespondingTile(testSourceCoords[index], It.IsAny<TileFormat>(), It.IsAny<bool>())).Returns(tile);
        this._dataFactoryMock.Setup(
                dataFactory => dataFactory.CreateDataSource(
      ...
      if (!isTargetNew)
      {
        targetDataMock.Setup(targetData => targetData.GetCorrespondingTile(testTargetCoord, It.IsAny<TileFormat>(), It.IsAny<bool>())).Returns(testTargetTile);
